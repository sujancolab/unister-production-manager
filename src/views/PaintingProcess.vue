        ...
        <ion-content class="ion-padding" :fullscreen="true">
            ...
            <div class="production-container">
                <!-- <div class="page-header">
                    ...
                    <h1 class="page-title">Painting Process</h1>
                </div> -->
                ...
                <ion-card>
                    <ion-card-header style="border-bottom: 1px solid lightgray;">
                        <ion-card-title>Allocate Painting Quantity</ion-card-title>
                    </ion-card-header>
                    <ion-card-content style="padding:0;">
                        <ion-grid>
                            <ion-row>
                ...
                <ion-card>
                    <ion-card-header style="border-bottom: 1px solid lightgray;">
                        <ion-card-title>Painting Process Records</ion-card-title>
                    </ion-card-header>
...
const processMode = 'painting';
const paintingStageOrder = [
    'auto_blasting_inspection',
    'semi_auto_blasting_inspection',
    'manual_blasting_inspection',
    'paint_inspection',
    'final_inspection'
];

const normalizePaintingStages = (rows) => {
    const normalizeKey = (row) => {
        if (row?.stage) return String(row.stage).trim().toLowerCase();
        if (row?.stage_name) {
    ...
    const filtered = (rows || [])
        .filter((el) => el?.type === 'inspection')
        .filter((el) => {
            const key = normalizeKey(el);
            return key !== 'black_inspection_stage' && key !== 'black_inspection';
        });

    const orderIndex = new Map(paintingStageOrder.map((key, idx) => [key, idx]));
...
const allocationStage = computed(() => {
    ...
    const inspectionStages = source.filter((s) => s?.type === 'inspection');
    return inspectionStages.find((s) => s?.stage === 'paint_inspection') || inspectionStages[0] || null;
});
...
const loadStages = async () => {
    ...
    try {
        ...
        // Only show painting inspection stages in correct order
        stages.value = normalizePaintingStages(rawStages);
...
const loadProcessRecords = async () => {
    ...
    try {
        const res = await api.get(`process-records?planning_id=${selectedPlanning.value}&mark_no=${encodeURIComponent(selectedMarkNo.value)}&mode=${processMode}`);
...
const openProcessRecord = async (record) => {
    ...
    try {
        ...
        const stageRows = normalizePaintingStages(rows);
